...
class ARModelTrainer(nn.Module):
    """
    ARModelTrainer class for training the VoxInstructAR model.
    """
    def __init__(
            self,
            config_path: str,
        ...
        """
        Initialize the ARModelTrainer class.
        ...
        """
        super(ARModelTrainer, self).__init__()
    ...
    def train_step(self):
        ...
        for _ in range(self.grad_accum_every):
            with self.accelerator.accumulate(self.model):
                ...
                logits = outputs.logits[:, hp.max_text_len:, :] # [b, t, n_token]
                loss_mask = sequence_mask(seq_lens - 1, max_len=seq_lens.max() - 1, device=device) 

                # Compute loss and backpropagate
                loss, acc = compute_loss(logits[:, :-1, :], seqs[:, 1:], mask=loss_mask, compute_acc=True, topk=(1,10))                   
        ...
        logs['samples'] = logs['samples']

        # log
        times = datetime.datetime.now() - start
        if steps % self.log_step == 0:         
            self.print("AR training! step: {}, loss: {:.4f}, acc_top1: {:4f}, acc_top10: {:4f}, lr: {}, samples: {}, min_len:{}, max_len: {}, grad_norm:{:4f}, times:{}\n".format(\
                        steps, logs['loss'], logs['acc_top1'], logs['acc_top10'], self.scheduler.get_last_lr(), logs['samples'], logs['min_len'], logs['max_len'], logs['grad_norm'], times))
        ...
        if self.is_main and self.val_step > 0 and steps % self.val_step == 0:
            ...
            for loaded_data in tqdm(self.val_loader):
                ...
                token_cnt += loss_mask.sum().item()
                val_loss += loss.item() * loss_mask.sum().item()
                all_acc_0 += acc[0].item() * loss_mask.sum().item()
                all_acc_1 += acc[1].item() * loss_mask.sum().item()
            ...
            self.print("AR validation! step: {}k, loss: {:.4f}, acc_top1: {:4f}, acc_top10: {:4f}\n".format(steps // 1000, val_loss, all_acc_0, all_acc_1))   
